...
// Time mean: 88.35, std: 250.12 | Amount mean: 94813.86, std: 47488.06 (other features unscaled = mean 0, scale 1)
...
function scaleFeatures(features) {
    ...
    console.log('Scaled sample:', scaled.slice(0,3), scaled.slice(-3));
    return scaled;
}
...
document.getElementById('predictionForm').addEventListener('submit', async (e) => {
    ...
    features[0] = parseFloat(timeEl?.value) || 0;
    // Amount index 29
    const amountEl = document.getElementById('amount');
    features[29] = parseFloat(amountEl?.value) || 0;
    // V1-V28 indices 1-28
    for (let i = 1; i <= 28; i++) {
        const vEl = document.getElementById('v' + i);
        features[i] = parseFloat(vEl?.value) || 0;
    }
